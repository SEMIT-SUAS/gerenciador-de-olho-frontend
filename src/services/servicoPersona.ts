...
export async function getAllPersona(): Promise<Persona[]> {
  try {
    const response = await fetch(
      `${API_BASE_URL}/persona-servico/listar-ativos`,
      {
        method: 'GET',
      },
    );
...
export async function getPersonaById(id: number): Promise<Persona> {
  try {
    const response = await fetch(
      `${API_BASE_URL}/persona-servico/buscar/${id}`,
    );
...
export async function changePersonaVisibility(
  id: number,
  visivel: boolean,
): Promise<void> {
  try {
    const response = await fetch(
      `${API_BASE_URL}/persona-servico/atualizar/visibilidade`,
      {
        method: 'PUT',
...
export async function changePersonaAtivo(
  id: number,
  ativo: boolean,
): Promise<void> {
  try {
    const response = await fetch(
      `${API_BASE_URL}/persona-servico/atualizar/atividade`,
      {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
