...
export function ListaCategorias() {
  ...
  return (
    <div>
      ...
      <button onClick={() => setShowAddForm(!showAddForm)}>+ Adicionar categoria</button>
      ...
      {showAddForm && (
        <form onSubmit={handleSubmit} style={{ marginTop: "1rem", marginBottom: "1rem" }}>
          ...
          <div>
            <label>Ícone: <input type="file" accept="image/*" onChange={e => handleFileChange(e)} required /></label>
            {icone && <p>Arquivo: {icone.name}</p>}
          </div>
          <div>
            <label>Ativo: <input type="checkbox" checked={ativo} onChange={e => setAtivo(e.target.checked)} /></label>
          </div>
          <div>
            <label>Visível: <input type="checkbox" checked={visivel} onChange={e => setVisivel(e.target.checked)} /></label>
          </div>
          <button type="submit">Salvar</button>
          <button type="button" onClick={() => setShowAddForm(false)} style={{ marginLeft: "0.5rem" }}>Cancelar</button>
        </form>
      )}
        ...
        <ul>
            {categorias.map((cat) => {
                ...
                return (
                <li key={cat.id} style={{ marginBottom: '10px', display: 'flex', alignItems: 'center' }}>
                    <img src={src} alt={cat.nome} width="40" style={{ marginRight: '10px' }} />
                    <span style={{ flexGrow: 1 }}>{cat.nome}</span>

                    <button onClick={() => handleToggleAtivo(cat)} style={{ marginRight: '5px' }}>
                    {cat.ativo ? 'Desativar' : 'Ativar'}
                    </button>

                    <button onClick={() => handleToggleVisivel(cat)} style={{ marginRight: '5px' }}>
        ...
        <div style={{
          ...
          <div style={{ background: '#fff', padding: '20px', borderRadius: '8px', width: '300px' }}>
            ...
            <form onSubmit={handleEditSubmit}>
              ...
              <button type="submit">Salvar</button>
              <button type="button" onClick={() => setShowEditModal(false)} style={{ marginLeft: '0.5rem' }}>
                Cancelar
              </button>
            </form>
          </div>
