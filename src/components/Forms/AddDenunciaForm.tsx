    ...
    return (
        <form onSubmit={handleSubmitForm} className="flex flex-col gap-4 px-4">
            <div className="flex flex-col gap-4 flex-1 overflow-y-autoo">
                <div>
                    ...
                    <p className="mt-1 text-sm text-gray-600">
                        Os campos de endereço abaixo são preenchidos automaticamente ao selecionar no mapa o local da denúncia. Porém você pode editá-los posteriormente.
                    </p>

                    <div className="grid grid-cols-1 gap-4 mt-4">
                        {["rua", "bairro", "pontoDeReferencia"].map((field) => (
                            <FormGroup key={field}>
                                <Label htmlFor={`address-${field}`}>{field === "pontoDeReferencia" ? "Ponto de referência" : field.charAt(0).toUpperCase() + field.slice(1)}</Label>
                                <FormInput
                                    id={`address-${field}`}
                                    name={field}
                                    ...
                                    placeholder={`Informe o ${field === 'pontoDeReferencia' ? 'ponto de referência' : field}`}
                                />
                                <FormInputError message={formErrors.address?.[field]?._errors?.[0]} />
                            </FormGroup>
                        ))}
                    </div>
                ...
                {formData.categoryId > 0 && (
                    <FormGroup>
                        ...
                        <FormInputError message={formErrors.categoryTipoId?._errors?.[0]} />
                    </FormGroup>
                )}
            ...
            >
                <button
                    type="button"
                    className={`w-full ${isSelectingNewDenunciaInMap ? 'bg-red-600 hover:bg-red-700' : 'bg-blue-600 hover:bg-blue-700'} text-white font-bold py-3 rounded-lg transition-colors cursor-pointer disabled:cursor-not-allowed`}
                    onClick={() => setIsSelectingNewDenunciaInMap(current => !current)}
                    disabled={isLoadingAddressSearch}
                >
                    {isLoadingAddressSearch ? 'Buscando...' : isSelectingNewDenunciaInMap ? 'Cancelar seleção' : 'Selecionar no mapa'}
                </button>
