...
export const DenunciaDetails: FC<DenunciaDetailsViewProps> = ({ item }) => {
    ...
    return (
        ...
        <div className="space-y-4">
            ...
            {item.acaoId ? (
                <>
                <div className="flex items-center px-4 py-3 justify-between border-1 border-yellow-600 rounded-xl">
                    <div>
                        <p className="text-sm font-semibold text-yellow-600">Ação Vinculada:</p>
                        <p className='text-md font-semibold text-yellow-800'>{acaoVinculada.length > 0 ? acaoVinculada[0].nome ?? acaoVinculada[0].id : 'Desconhecida'}</p>
                        <p className='text-xs font-semibold text-yellow-700'>{acaoVinculada.length > 0 ? acaoVinculada[0].secretaria ?? acaoVinculada[0].id : 'Desconhecida'}</p>
                    </div>
                    
                    <button onClick={() => setIsOpen(true)} className='z-10 cursor-pointer rounded-full p-2 text-yellow-800 transition-colors hover:bg-red-100 hover:text-red-600' ><FaTrashAlt /></button>
                ...
                <div className=" py-3 px-4 rounded-xl border border-gray-200">
                    ...
                    <div className="py-2">
                        <button onClick={() => startLinking(item)} className="w-full bg-blue-600 text-white text-sm font-semibold py-2 rounded-lg hover:bg-blue-700 transition-colors">
                            Vincular a uma Ação 
                        </button>
                    </div>
                </div>
